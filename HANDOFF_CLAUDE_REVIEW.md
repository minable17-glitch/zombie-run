...
- 쿼리 검증: 로컬 PGlite에서 전체 마이그레이션 적용 시 6행 모두 `true`, 세 파일을 하나씩 빼면 해당 파일 행만 `false`가 되는 것을 확인했다.
- **운영 결과 (2026-09-26, 사용자가 SQL Editor에서 직접 실행): 6행 모두 `true`.** 세 파일의 함수·권한·제약은 운영에 반영된 상태다. 추가로 적용할 SQL 없음. 이 쿼리가 검사하지 않는 다른 객체의 상태는 여전히 이 결과로 보증되지 않는다.
...
5. §3 운영 설정(Auth Redirect URL, Site URL, Edge Function secret, Cron 상태)의 현재 상태 대조. (§2 SQL 3개는 2026-09-26 운영 반영 확인 완료)
