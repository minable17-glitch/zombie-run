...
export const changeRunnerCode = code => call('zr_runner_change_code', { p_code:code })
const mode = config => config.mapId ? `map:${config.mapId}` : config.playMode || 'free'
export const startSolo = (id, config) => call('zr_solo_start', { p_id:id, p_mode:mode(config), p_pace:config.paceIdx, p_radius:config.mapId ? 0 : config.radiusIdx ?? 1 })
...
export const soloBoard = config => call('zr_solo_board', { p_mode:mode(config), p_pace:config.paceIdx, p_radius:config.mapId ? 0 : config.radiusIdx ?? 1 })
